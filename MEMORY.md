...
- **Pi Agent**: 已参考 OpenCode 完成 Pi Coding Agent (`~/.pi/agent`) 配置与深度主题/TUI 重构；安装 `pi-zentui` 扩展并配置 `zentui.json`（固定底栏输入框、OpenCode 边框与模型元数据轨、Starship 式 Git/环境状态栏、Arch 专用 Nerd 图标与百分比量规）；主题生成函数 `write_pi` 已扩展至 `~/.config/noctalia/morandi-gen.py`，全套莫兰迪 TUI 配色随壁纸动态同步
