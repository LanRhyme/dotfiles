...
- **Antigravity**：AUR 包滞后需手动维护（git clone AUR 仓库改 pkgver/_build + updpkgsums + makepkg -f，本地副本 `~/tmp/antigravity-pkgbuild/`，下载 URL 形如 storage.googleapis.com/antigravity-public/antigravity-hub/<ver>-<build>/linux-x64/Antigravity.tar.gz）；agy CLI 独立 ELF（~/.local/bin/agy，agy update 自管理不随 pacman）；`antigravity --version` 输出的是内置 node 版本非应用版本
