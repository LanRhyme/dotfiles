...
- **Denial**：`denial 0.2.16-1` + `denial-flutter-engine`（基于 Flutter + Rust 的 Wayland 合成器）；已在 `/etc/pacman.conf` 配置 `[denial]` 官方源并导入 GPG 密钥；入口 `/usr/bin/denial-session`（`/usr/share/wayland-sessions/denial.desktop`）
