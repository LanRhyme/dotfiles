...
- 已修复 Alacritty 终端偏蓝问题并完全集成到莫兰迪主题引擎: 修复了 `morandi-gen.py` 中 `write_alacritty` 使用硬编码基础色且未清理 `themes/noctalia.toml` 中残留 Catppuccin Mocha 高饱和蓝色（`#89B4FA`）的问题；重构了 `write_alacritty(palette)`，将 ANSI 16 色全量绑定并优化为阶梯式莫兰迪调色板（引入 `peach` 杏橙、`foam` 薄荷青、`subtext` 白等分层调色），使 `normal` 与 `bright` 色彩层次清晰、丰富且符合莫兰迪暖调美感
