...
### 5. `android-hide-app`
* **功能**: Android 设备应用防检测环境全自动配置工具
* **使用方法**: 
  - 列出保护列表: `android-hide-app -l`
  - 注入防检测: `android-hide-app <包名>`（支持传入多个）
  - 移除防检测: `android-hide-app -r <包名>`
* **特性**:
  - 全流程纯命令行与静默配置文件写入，绝不触发模拟触摸
  - 自动管理 HMA-OSS 双路径配置（`/data/misc/...` 与应用私有目录）并自适应权限与 SELinux 标签
  - 自动同步 Tricky Store 目标名单并平滑重载底层服务
